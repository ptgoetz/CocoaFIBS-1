...
#import "MGToolbarController.h"
#include "MGGameView.h"
#include "MGTerminalWindowController.h"
#include "MGLoginWindowController.h"
#include "MGChatController.h"
#include "MGGameModel.h"
#include "CFLog.h"
...
- (id)init 
{
	...
	theAGFIBSSocket = [[MGSocketStream alloc] init];
	...
	switch(cookie){
	   ...
	   case FIBS_PreLogin: {
		...
		case CLIP_YOU_SHOUT:{
			...
			MGGameController *game = [[MGGameController alloc] init];
...
{
	if (!thePrefWindow) {
		thePrefWindow = [[MGPrefController alloc] init];
	}
...
{
	if (!terminalWindow) {
		terminalWindow = [[MGTerminalWindowController alloc] init];
	}
