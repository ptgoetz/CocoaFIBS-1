...
-(void)chipFollowsMouseWhileDragging 
{
    ...
    [chipImages[draggedChipOwnedBy] compositeToPoint:chipRect.origin operation:NSCompositingOperationSourceOver];
}
...
//            [nc postNotificationName:@"AGFIBSDisplaySystemMsg" object:@""];
	...
	MGTriangle *selectedTriangle = [self determineTriangleFromPoint:[self convertPoint:[theEvent locationInWindow] fromView:nil]];
...
- (void)autoMoveFromTriangle:(MGTriangle *)fromTriangle
{
		...
		MGTriangle *toTrianglePossability1 = nil;
		MGTriangle *toTrianglePossability2 = nil;
		MGTriangle *toHomeTrianglePossability = [theAGFIBSGameModel playerHome];
...
- (void)autoDoubleMoveFromTriangle:(MGTriangle *)fromTriangle
{
		int fromTriangleArrayPos = [self pipNumToArrayPos:[fromTriangle pipNumber]];
...
-(MGTriangle *)determineTriangleFromPoint:(NSPoint)aPoint 
{
	int i;
	NSRect myRectTop;
	NSRect myRectBottom;
...
- (void)setHighlightStatusOfTriangles
{
	int i;
	for (i = 0; i < NUMBER_OF_TRIANGLES; i++) {
		MGTriangle *potentialMoveToTriangle = [[theAGFIBSGameModel gameBoard] objectAtIndex:i];
...
- (void)highlightTriangles
{
	...
	MGTriangle *potentialMoveToTriangle;
...
{
	MGTriangle *selectedTriangle = [self determineTriangleFromPoint:mouseLocation];
...
-(void)placeChip:(MGTriangle *)selectedTriangle 
{
...
-(BOOL)canMoveToTriangle:(MGTriangle *)toTriangle 
{
...
//		NSLog(@"Failed: not home yet");
		...
		MGTriangle *inbetweenStepTriangle1 = [[theAGFIBSGameModel gameBoard] objectAtIndex:[self pipNumToArrayPos:inbetweenJump1PipNum]];
...
//			NSLog(@"yes2");
		...
		for (i = 0; i < moveType-1; i++) {
			MGTriangle *inbetweenStepTriangle = [[theAGFIBSGameModel gameBoard] objectAtIndex:[self pipNumToArrayPos:inbetweenJumpPipNum]];
...
- (MGGameModel *)theAGFIBSGameModel { return [[theAGFIBSGameModel retain] autorelease]; }
- (void)setTheAGFIBSGameModel:(MGGameModel *)newTheAGFIBSGameModel
{
