...
- (id)copyWithZone:(NSZone *)zone {
	MGGameModel *copy = [[MGGameModel alloc] init];
...
- (void)newGame {
	...
	playerDice = [[MGDice alloc] initWithDie:0 otherDie:0];
	opponentDice = [[MGDice alloc] initWithDie:0 otherDie:0];
	
	playerDiceFromLastTurn = [[MGDice alloc] initWithDie:6 otherDie:6];
	opponentDiceFromLastTurn = [[MGDice alloc] initWithDie:5 otherDie:6];
	
	playerBar = [[MGTriangle alloc] initWithOwnedBy:1 numberOfChips:0 pipNumber:BAR_PIP_NUMBER];
	opponentBar = [[MGTriangle alloc] initWithOwnedBy:2 numberOfChips:0 pipNumber:BAR_PIP_NUMBER];
	playerHome = [[MGTriangle alloc] initWithOwnedBy:1 numberOfChips:0 pipNumber:HOME_PIP_NUMBER];
	opponentHome = 0;
	int i,j;
	j = NUMBER_OF_TRIANGLES;
	for (i = 0; i < NUMBER_OF_TRIANGLES; i++) {		
		MGTriangle *aTriangle = [[MGTriangle alloc] initWithOwnedBy:ownedBySetup[i] numberOfChips:numberOfChipsSetup[i] pipNumber:j];
...
- (void)updateModelFromFIBS_Board 
{
	...
	if ([[NSUserDefaults standardUserDefaults] boolForKey:@"highestDiceFirst"] && playerDie1 < playerDie2) {
		playerDice = [[MGDice alloc] initWithDie:playerDie2 otherDie:playerDie1];
	}
	else {
		playerDice = [[MGDice alloc] initWithDie:playerDie1 otherDie:playerDie2];
	}
	[self setPlayerDice:playerDice];

	[opponentDice release];
	opponentDice = [[MGDice alloc] initWithDie:[[fibsBoardStateDictionary objectForKey:@"opponentDie1"] intValue] otherDie:[[fibsBoardStateDictionary objectForKey:@"opponentDie2"] intValue]];
	
	if ([[fibsBoardStateDictionary objectForKey:@"playerDie1"] intValue] > 0) {
		playerDiceFromLastTurn =  [[MGDice alloc] initWithDie:[[fibsBoardStateDictionary objectForKey:@"playerDie1"] intValue] otherDie:[[fibsBoardStateDictionary objectForKey:@"playerDie2"] intValue]];
	}
	else if ([[fibsBoardStateDictionary objectForKey:@"opponentDie1"] intValue] > 0) {
		opponentDiceFromLastTurn = [[MGDice alloc] initWithDie:[[fibsBoardStateDictionary objectForKey:@"opponentDie1"] intValue] otherDie:[[fibsBoardStateDictionary objectForKey:@"opponentDie2"] intValue]];
	}
...
- (MGDice *)playerDice { return [[playerDice retain] autorelease]; }
- (void)setPlayerDice:(MGDice *)aPlayerDice
{
...
- (MGDice *)opponentDice { return [[opponentDice retain] autorelease]; }
- (void)setOpponentDice:(MGDice *)anOpponentDice
{
...
- (MGDice *)playerDiceFromLastTurn { return [[playerDiceFromLastTurn retain] autorelease]; }
- (void)setPlayerDiceFromLastTurn:(MGDice *)aPlayerDiceFromLastTurn
{
...
- (MGDice *)opponentDiceFromLastTurn { return [[opponentDiceFromLastTurn retain] autorelease]; }
- (void)setOpponentDiceFromLastTurn:(MGDice *)anOpponentDiceFromLastTurn
{
...
- (MGTriangle *)playerBar { return [[playerBar retain] autorelease]; }
- (void)setPlayerBar:(MGTriangle *)aPlayerBar
{
...
- (MGTriangle *)opponentBar { return [[opponentBar retain] autorelease]; }
- (void)setOpponentBar:(MGTriangle *)anOpponentBar
{
...
- (MGTriangle *)playerHome { return [[playerHome retain] autorelease]; }
- (void)setPlayerHome:(MGTriangle *)aPlayerHome
{
...
- (MGTriangle *)draggedFromTriangle { return [[draggedFromTriangle retain] autorelease]; }
- (void)setDraggedFromTriangle:(MGTriangle *)aDraggedFromTriangle
{
...
- (MGTriangle *)draggedToTriangle { return [[draggedToTriangle retain] autorelease]; }
- (void)setDraggedToTriangle:(MGTriangle *)aDraggedToTriangle
{
